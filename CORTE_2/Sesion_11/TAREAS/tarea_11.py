...
def main():
    ...
    while opc != 'y':  # Cambiar la condición para salir cuando se ingrese 'y'
        per = Imc()  # Crear una instancia de la clase Imc
        per.nombre = input('Nombre: ')
        per.apellido = input('Apellido: ')
        per.edad = input('Edad: ')  # Agregar la entrada de la edad
        per.estatura = float(input('Estatura (en cm): '))  # Cambiar a float
        per.peso = float(input('Peso (en kg): '))  # Cambiar a float
        Personas.append(per)
        opc = input('¿Desea agregar otra persona? (y/n): ')

    print('-------IMC -----\n')
    for i in Personas:
        imc = i.calculo()  # Usar 'i' en lugar de 'per'
        resultado_imc = i.tabla_de_imc(imc)  # Usar 'i' en lugar de 'per'
